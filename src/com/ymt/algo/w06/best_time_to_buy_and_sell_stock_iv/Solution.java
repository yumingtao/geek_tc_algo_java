package com.ymt.algo.w06.best_time_to_buy_and_sell_stock_iv;
            ...
            for (int j = 0; j <= 1; j++) {
                ...
                for (int c = 0; c <= k; c++) {
                    ...
                    if (j == 0 && c > 0) {
                        ...
                        f[i][0][c] = Math.max(f[i][0][c], f[i - 1][1][c - 1] + adjustedPrices[i]);
                    }
                    ...
                    if (j == 1) {
                        ...
                        f[i][1][c] = Math.max(f[i][1][c], f[i - 1][0][c] - adjustedPrices[i]);
                    }
                }
            }
        }
